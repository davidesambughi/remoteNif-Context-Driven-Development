...
export function getAdminSlaBreachSubject(orderId: string): string {
  return `Express SLA breach — Order #${orderId.slice(0, 8)}`
}

const TIER_LABEL = 'Express'
...
const styles = {
  ...
  heading: { fontSize: '20px', fontWeight: '700', color: '#dc2626', margin: '0 0 20px 0' },
  label: {
    fontSize: '11px',
    fontWeight: '600',
    color: '#64748b',
...
export function AdminSlaBreachEmail({
  orderId,
  customerName,
  hoursOverdue,
  adminOrderUrl,
}: AdminSlaBreachEmailProps) {
  return (
    <Html lang="en">
      <Head />
      <Preview>Express SLA breach — {customerName}, Order #{orderId.slice(0, 8)}</Preview>
      <Body style={styles.body}>
        <Container style={styles.container}>
          ...
          <Section>
            <Text style={styles.heading}>Express SLA has been breached.</Text>
            ...
            <Text style={styles.value}>{TIER_LABEL}</Text>

            <Text style={styles.label}>Overdue by</Text>
            <Text style={styles.value}>{hoursOverdue} hours</Text>
