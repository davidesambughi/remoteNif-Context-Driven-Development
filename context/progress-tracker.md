...
- **Feature 19c — Code Quality: Remaining Yellow Smells** — 6 findings resolved: (1a) `lib/email/send.ts` `any` replaced with `ReactElement` imported from `'react'`; (6a) `'use server'` directive removed from `send.ts` (it is a utility, not a Server Action); (6c) `throw new Error` inside the `try/catch` in both `createCheckoutSession` and `createRenewalCheckoutSession` replaced with early `return { success: false, ... }` — no behaviour change; (3d) status labels already resolved in 19d; (2b) `interface DocumentRecord` in `PersonalDetailsForm.tsx` replaced with `type DocumentRecord = Pick<SelectDocument, ...>` — type-only import, no runtime change; (2c) `SlotStatus` exported from `DocumentUploadSlot.tsx`, imported in `PersonalDetailsForm.tsx`, local duplicate removed. Build clean, 436 unit tests passing.
