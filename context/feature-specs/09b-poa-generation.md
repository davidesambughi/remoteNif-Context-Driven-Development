...
| Body text | `var(--text-primary)` | `text-primary` |
| Supporting text | `var(--text-secondary)` | `text-secondary` |
| Muted text | `var(--text-muted)` | `text-muted` |
| Default border | `var(--border-default)` | `border-default` |
...
- PDF generation and storage operations are extracted to `lib/pdf/generator.ts` — this is business logic and must not live in the Server Action.
- `generatePoa` Server Action goes in `app/actions/orders.ts` — thin: validate → auth → fetch → call `lib/pdf/generator.ts` → update DB → return signed URL.
...
All external input entering a Server Action must be validated with Zod at the boundary before any logic runs (Invariant 6, code-standards.md).

`generatePoa` requires this schema (define it at the top of the action file or co-locate in `lib/validations/orders.ts`):
...
Personal details are already validated by `PersonalDetailsSchema` when saved. The action must additionally verify all required personal detail fields are non-null before generating (return early with an error if any are missing — safety check for data integrity).
...
The POA section lives at the bottom of the summary card (saved state in `PersonalDetailsForm`), separated from the details grid by a horizontal divider. It is not a separate card.
...
**Regenerate needed state** (user edited details, re-saved, `poaSignedUrl` prop is null but `poaUrl` local state was previously set):
...
- Client identification block — all personal detail fields with bilingual labels (Portuguese first, English second — e.g. "Nome completo / Full legal name:"). **This is the permanent design — not a draft approach.**
- Authorization paragraph — alternating EN and PT paragraphs (both present permanently — this is a bilingual legal document, not locale-driven)
...
- Footer — bilingual: "Gerado por RemoteNIF / Generated by RemoteNIF · remotenif.com" left side; "Requer assinatura manuscrita / Requires handwritten signature before submission." right side
...
4. Create `lib/pdf/poa-template.tsx`:
   ...
   - All draft placeholder copy is hardcoded in this file — no i18n (PDF is a legal document, not locale-specific)
   - Mark draft sections clearly with "[DRAFT — REPLACE WITH LEGAL COPY]" inline

5. Create `lib/pdf/generator.ts` — extracts all PDF and storage business logic out of the action:
   - Export `generateAndStorePoaPdf(orderId: string, details: PersonalDetailsData, supabase: SupabaseClient): Promise<{ path: string; signedUrl: string }>`
   - Steps inside this function:
     a. Render PDF to buffer: `await renderToBuffer(<PoaDocument {...details} generatedDate={...} />)`
     b. Upload buffer to Supabase Storage: `supabase.storage.from('documents').upload('poa/{orderId}/poa-draft.pdf', buffer, { contentType: 'application/pdf', upsert: true })`
     c. Throw a typed error on storage failure (the action catches this and returns the appropriate `ActionResult`)
     d. Generate a 1-hour signed URL: `supabase.storage.from('documents').createSignedUrl('poa/{orderId}/poa-draft.pdf', 3600)`
     e. Return `{ path: 'poa/{orderId}/poa-draft.pdf', signedUrl }`
   - Export `deleteStoredPoaPdf(path: string, supabase: SupabaseClient): Promise<void>` — wraps `supabase.storage.from('documents').remove([path])`; used by `savePersonalDetails` when regeneration is required
   - Export `getPoaSignedUrl(path: string, supabase: SupabaseClient): Promise<string | null>` — wraps `createSignedUrl`; used by the dashboard page RSC; returns null on failure

6. Add two queries to `lib/db/queries.ts`:
   - `updateOrderPoaPath(orderId: string, userId: string, path: string | null): Promise<void>` — updates `poaGeneratedPath` on the order, ownership check via `and(eq(orders.id, orderId), eq(orders.userId, userId))`
   - `getOrderPersonalDetails(orderId: string, userId: string)` — fetches the order's personal detail fields needed for PDF generation (fullName, dateOfBirth, nationality, passportNumber, passportExpiry, address, poaGeneratedPath); returns null if not found or not owned by user

7. Add `generatePoa` Server Action to `app/actions/orders.ts`:
   - Signature: `generatePoa(orderId: string): Promise<ActionResult<{ signedUrl: string }>>`
   - Steps:
     a. Validate input: `GeneratePoaSchema.safeParse({ orderId })` — return error result if invalid
     b. Auth check via `requireAuth()`
     c. Fetch order personal details via `getOrderPersonalDetails(orderId, user.id)`
     d. Return `{ success: false, error: 'not_found' }` if order not found
     e. Validate all required fields are non-null — return `{ success: false, error: 'incomplete_details' }` if any are missing
     f. Call `generateAndStorePoaPdf(orderId, details, supabase)` from `lib/pdf/generator.ts`
     g. On failure: return `{ success: false, error: 'storage_error' }`
     h. Update `orders.poaGeneratedPath` via `updateOrderPoaPath(orderId, user.id, path)`
     i. Return `{ success: true, data: { signedUrl } }`

8. Update `savePersonalDetails` Server Action in `app/actions/orders.ts`:
   - After successfully updating the order's personal details, fetch the current `poaGeneratedPath` from the order
   - If non-null: call `deleteStoredPoaPdf(poaGeneratedPath, supabase)` from `lib/pdf/generator.ts`, then call `updateOrderPoaPath(orderId, user.id, null)` to clear the path
   - This forces regeneration after any edit

9. Update `app/[locale]/(dashboard)/dashboard/page.tsx`:
   - After fetching the active order, if `order.poaGeneratedPath` is non-null, call `getPoaSignedUrl(order.poaGeneratedPath, supabase)` from `lib/pdf/generator.ts`
   - Pass the resulting `poaSignedUrl: string | null` as a prop to `PersonalDetailsForm`

10. Update `PersonalDetailsFormProps` in `components/dashboard/PersonalDetailsForm.tsx`:
    ...
    - In the saved summary view, below the `<dl>` grid, add a horizontal divider and the POA section (three states described in Design)
    ...
    - When `isSaved` flips back to `false` (user clicks Edit): clear `poaUrl` to null

11. Add the new `poa` i18n keys to `messages/en.json`, `fr.json`, `es.json`, `de.json` under `personalDetails` (see i18n section for the key shape — use English values in all four files for now).
...
(The package ships its own TypeScript types — do not install `@types/react-pdf`.)
...
- Do not make the POA locale-aware — it is a static bilingual PT/EN document regardless of user locale. Portuguese is primary (legally binding); English is secondary (reference). This is the permanent design decision.
...
- `generatePoa` Server Action validates `orderId` with Zod before any logic runs.
- PDF rendering and storage operations live in `lib/pdf/generator.ts`, not in the action.
- `generatePoa` generates a PDF, stores it at `poa/{orderId}/poa-draft.pdf` in the `documents` bucket, and returns a signed URL.
