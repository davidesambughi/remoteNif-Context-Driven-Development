...
- `lib/email/templates/` — one `.tsx` file per email template. Templates are React Email components. No UI logic, no Tailwind — use React Email primitives only (`@react-email/components`).
...
Email templates use React Email primitives only (`Html`, `Head`, `Body`, `Container`, `Section`, `Text`, `Button`, `Hr`, `Link`). No Tailwind. No design tokens — email clients don't support CSS variables.
...
- Brand accent: `#3b82f6` (matches `--brand-primary` approximately — hardcoded for email)
...
3. Create `lib/email/send.ts`:
   ...
   - Export async `sendEmail(to: string, locale: EmailLocale, payload: EmailPayload): Promise<void>`:
     ...
     - Renders the correct React Email template using `render()` from `@react-email/components`
     - Gets the subject from the template's colocated subject helper
     - Calls `resendClient.emails.send({ from: env.RESEND_FROM_EMAIL, to, subject, html })`
     - On error: `console.error('[sendEmail]', error)` — never throws

4. Create `lib/email/templates/order-confirmation.tsx`:
   - Export `OrderConfirmationEmail` as a React Email component
   - Props: `{ locale: EmailLocale; orderId: string; tier: string; amountEur: string; dashboardUrl: string }`
   - Export `getOrderConfirmationSubject(locale: EmailLocale, orderId: string): string` from the same file — `sendEmail` calls this to get the subject
...
5. Add `getUserLanguage` query to `lib/db/queries.ts`:
   - Signature: `getUserLanguage(userId: string): Promise<EmailLocale>`
   - Fetches `users.language` for the given `userId`; returns `'en'` if not found or null

6. Update the Stripe webhook handler in `app/api/webhooks/stripe/route.ts` (inside `handleCheckoutSessionCompleted`):
   - After the order and payment records are inserted, fetch the user's language via `getUserLanguage(userId)`
   - Call `await sendEmail(customerEmail, locale, { template: 'order_confirmation', orderId, tier, amountEur })` — fire after the transaction commits, not inside it
   - `customerEmail` is available from the Stripe session object (`session.customer_details?.email ?? session.customer_email`)
   - `amountEur` is derived from `session.amount_total` (divide by 100, prepend `€`, e.g. `"€129"`)
...
- `lib/email/send.ts` exports `sendEmail`, `EmailLocale`, and `EmailTemplateName`.
- `lib/email/templates/order-confirmation.tsx` exists and renders without error when passed valid props.
- `getUserLanguage` query exists in `lib/db/queries.ts` and returns `'en'` as a safe default when the field is null.
- Stripe webhook handler calls `sendEmail` for `order_confirmation` after a successful checkout — verify by completing a test checkout and confirming the email is received at the test address.
